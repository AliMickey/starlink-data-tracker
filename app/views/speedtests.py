            ...
            if dbCheck == 0: # If speedtest result does not exist in db
                ...
                if dataScript:
                    ...
                    if data['isp_name'] == "SpaceX Starlink": # If ISP is Starlink
                        ...
                        elif int(data['latency']) <= 5 or int(data['download']) >= 600000 or int(data['download']) <= 500 or int(data['upload']) >= 70000 or int(data['upload']) <= 500: # If test results are not within a valid range (<5ms latency, 1-600mbps download, 0.5-50mbps upload) for Starlink (may change in the future)
                            error = "Speedtest contains potentially inaccurate results. Please try again.\nLimits: Latency (> 5ms), Download (600Mbps - 0.5Mbps), Upload(70Mbps - 0.5Mbps)."
                        else: # Add speedtest             
